...
(defun dotspacemacs/user-config/get-mode-hook (mode)
  (intern-soft (concat (symbol-name mode) "-hook")))

(defun dotspacemacs/user-config/line-number-default ()
  (let* ((relative '(org-mode
                     haskell-mode
                     c-mode
                     markdown-mode
                     ott-mode
         ...
         (absolute '()))
    (progn
      (dolist (mode relative)
        (let* ((mode-hook (dotspacemacs/user-config/get-mode-hook mode)))
          (when mode-hook
            (add-hook mode-hook 'spacemacs/toggle-relative-line-numbers-on))
          )
        )
      (dolist (mode absolute)
        (let* ((mode-hook (dotspacemacs/user-config/get-mode-hook mode)))
          (when mode-hook
            (add-hook mode-hook 'spacemacs/toggle-absolute-line-numbers-on))
          )
        )
      )
    )
...
before packages are loaded."
  (add-hook 'haskell-mode-hook 'haskell-mode-user-setup)
  ...
  (setq-default idris-interpreter-path "idris2")
